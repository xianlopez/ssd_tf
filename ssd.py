...
class FeatureMap:
    def __init__(self, anchor_boxes):
        ...
        self.grid_size = anchor_boxes.shape[1]
        self.nanchors = anchor_boxes.shape[0]
        self.nboxes = self.nanchors * self.grid_size * self.grid_size
...
class ssd_net:
    def __init__(self, config, nclasses):
        ...
        self.feature_maps = self.create_feature_maps()
    ...
    def encode_box(self, box, anchor_idx):
        ...
        anchor = self.vec_anchors[anchor_idx, :]
    ...
    def encode_gt(self, gt_boxes):
        ...
        iou = compute_iou_flat(gt_vec, self.vec_anchors) # (n_gt, nboxes)
    ...
    def create_feature_maps(self):
        nmaps = len(self.cfg.anchor_ratios)
        feature_maps = []
        self.vec_anchors = []
        for map_idx in range(nmaps):
            feat_map = self.create_anchors_for_feature_map(self.cfg.grid_sizes[map_idx], self.cfg.anchor_sizes[map_idx],
                                                           self.cfg.anchor_sizes[map_idx + 1], self.cfg.anchor_ratios[map_idx],
                                                           self.cfg.image_size, self.cfg.image_size, self.cfg.steps[map_idx])
            ...
            for col in range(feat_map.grid_size):
                for row in range(feat_map.grid_size):
                    for anc_idx in range(feat_map.nanchors):
                        self.vec_anchors.append(feat_map.anchor_boxes[anc_idx, row, col, :])
        self.vec_anchors = np.array(self.vec_anchors)
    ...
    def net(self,
        ...
        with tf.variable_scope(scope, 'vgg_16', [inputs], reuse=reuse):
            ...
            softmax = tf.zeros((batch_size, 0, self.nclasses + 1), tf.float32)
            logits = tf.zeros((batch_size, 0, self.nclasses + 1), tf.float32)
            localizations = tf.zeros((batch_size, 0, 4), tf.float32)
            for i, layer in enumerate(feat_layers):
                nanchors = self.feature_maps[i].nanchors
                grid_size = self.feature_maps[i].grid_size
                nboxes_feat_map = nanchors * grid_size * grid_size
                with tf.variable_scope(layer + '_mbox'):
                    if layer == 'conv4':
                        print('Adding ssd_multibox_layer to conv4')
                    logits_this_map, locs_this_map = self.ssd_multibox_layer(end_points[layer], nanchors, grid_size, layer)
                softmax_this_map = tf.nn.softmax(logits_this_map, axis=-1)
                logits_this_map = tf.reshape(logits_this_map, [-1, nboxes_feat_map, self.nclasses + 1])  # (batch_size, nboxes_feat_map, nclasses + 1)
                logits = tf.concat([logits, logits_this_map], axis=1)
                softmax_this_map = tf.reshape(softmax_this_map, [-1, nboxes_feat_map, self.nclasses + 1])  # (batch_size, nboxes_feat_map, nclasses + 1)
                softmax = tf.concat([softmax, softmax_this_map], axis=1)
                locs_this_map = tf.reshape(locs_this_map, [-1, nboxes_feat_map, 4])  # (batch_size, nboxes_feat_map, 4)
            ...
            predictions = tf.concat([localizations, softmax], axis=2)  # (batch_size, nboxes, 4 + nclasses + 1)

            return net_output, predictions, end_points


    def ssd_multibox_layer(self, inputs, nanchors, grid_size, layer_name):
    ...
    def create_anchors_for_feature_map(self, grid_size, box_size, next_size, ratios, img_width, img_height, step):
        grid_like_ssd_paper = True
        if grid_like_ssd_paper:
            grid = np.linspace(0.5 * step, (0.5 + grid_size - 1) * step, grid_size) / float(img_width)
        else:
        ...
        if 1 in ratios and next_size is not None:
            ...
            sizes = np.zeros(nanchors)
            sizes[:] = box_size
        anchor_boxes = np.zeros(shape=(nanchors, grid_size, grid_size, 4), dtype=np.float32)
        for i in range(len(ratios)):
            width = sizes[i] * np.sqrt(ratios[i])
            height = sizes[i] / np.sqrt(ratios[i])
            for row in range(grid_size):
                for col in range(grid_size):
                    anchor_boxes[i, row, col, 0] = grid[row]
                    anchor_boxes[i, row, col, 1] = grid[col]
                    anchor_boxes[i, row, col, 2] = width
                    anchor_boxes[i, row, col, 3] = height
        return FeatureMap(anchor_boxes)
